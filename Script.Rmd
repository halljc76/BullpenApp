...
spinVeloGraphs <- function(pitcher, sessionDate, spinVeloFlag) {
  
  df <- queryDB(conn, 
        paste("SELECT ROW_NUMBER() OVER(PARTITION BY TaggedPitchType) AS 
               Row, TaggedPitchType, ZoneSpeed, SpinRate
  ...
  if (spinVeloFlag) {
    return(
          plot_ly(data = df %>% group_by(df$TaggedPitchType),
                  ...
                  mode = "lines+markers",
                  hoverinfo = "text",
                  text = ~paste(df$Row, ": ", round(df$ZoneSpeed, 1), " 
                                MPH", 
                                sep = "")) %>% 
                  layout(xaxis = list(            
                         tickvals = ~df$Row,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$sessionSelect, 
   ...
   output$szDisplay <- renderPlotly(
                 expr = {
                   locationDist(flags$ps, T, flags$ss)
                 }
               )
               
   output$veloDisplay <- renderPlotly(
     expr = {
       spinVeloGraphs(input$playerSelect, 
                      input$sessionSelect, T)
     }
   )
   ...
   output$spinDisplay <- renderPlotly(
     expr = {
       spinVeloGraphs(input$playerSelect, 
                      input$sessionSelect, F)
     }
   )
   
   output$relDisplay <- renderPlotly(
     expr = {
       releasePoints(input$playerSelect, 
                     input$sessionSelect)
     }
   )
