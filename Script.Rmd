...
summaryStats <- function(pitcher) {
  df <- locationDist(F, T, pitcher, F)
  ...
  pct <- round(pct / tot, digit = 2)
  avgVelos <- round(avgVelos, digit = 2)
  avgSpins <- round(avgSpins, digit = 0)
...
locationDist <- function(warmupFlag, liveFlag, pitcher, plotBool) {
               ...
               output$szDisplay <- renderPlotly(
                 expr = {
                   if (input$warmupSelect && input$liveSelect) {
                     locationDist(T, T, flags$ps, T)
                   }
                   else if (input$liveSelect && !(input$warmupSelect)) {
                     locationDist(F, T, flags$ps, T)
                   }
                   else {
                     locationDist(T, F, flags$ps, T)
                   }
                 }
               )
  )
