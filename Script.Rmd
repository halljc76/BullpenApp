...
summaryStats <- function(pitcher, sessionDate) {
          ...
          query = gsub(pattern = '_', replacement = pt,
                       x = gsub(pattern = '--',replacement = pitcher,
                                x = gsub(pattern = '---', replacement = sessionDate,
                                         # Query: Find the number of pitches thrown.
                                         x = "SELECT COUNT(*) 
                                               FROM TestData
      ...
      as.list(
        queryDB(
          ...
          query = gsub(pattern = '_', replacement = pt,
                       x = gsub(pattern = '--',replacement = pitcher,
                                x = gsub(pattern = '---', replacement = sessionDate,
                                         # QUERY: Find avg. speed of each pitch type.
      ...
      as.list(
        queryDB(
          ...
          query = gsub(pattern = '_', replacement = pt,
                       x = gsub(pattern = '--',replacement = pitcher,
                                x = gsub(pattern = '---', replacement = sessionDate,
                                         # QUERY: Find avg. spin of each pitch type.
  ...
  ret <- data.frame(pitches = names, counts = cts, 
                    avgVelos = avgVelos, avgSpins = avgSpins)
    colnames(ret) <- c("Pitch", "Num. Thrown", "Avg. Velo", "Avg. Spin")
  return(ret)
}
  ...
  if (warmupFlag == FALSE && liveFlag == TRUE) {
    df <- queryDB(conn, 
                  gsub(pattern = "_",
                       replacement = pitcher,
                       x = gsub(pattern = '--', replacement = sessionDate,
                                # QUERY: Get Location, Type, Tilt, and Velo.
                               x = "SELECT PlateLocSide, PlateLocHeight, 
                                           TaggedPitchType, Tilt, ZoneSpeed
                                   FROM TestData
...
spinVeloGraphs <- function(warmupFlag, liveFlag, pitcher, sessionDate, spinVeloFlag) {
  if (!warmupFlag && liveFlag) {
    # QUERY: Get the distinct pitch types thrown.
    ...
    for (pt in unname(pts)[[1]]) {
      # QUERY: Get the row index, pitch type, velo, and spin.
      temp <- queryDB(conn, paste("SELECT ROW_NUMBER() OVER() AS Row, TaggedPitchType,
                                  ZoneSpeed, SpinRate
                             FROM TestData 
                            WHERE Pitcher = '", pitcher, 
  ...
  if (warmupFlag == liveFlag) {
    ...
    df <- queryDB(conn, 
            paste("SELECT RelSide, RelHeight, 
                    ...
                    WHERE Pitcher = '", pitcher, "'",
                  " AND Date = '", sessionDate, "'",
                  " AND TaggedPitchType != 'Warmup'", sep = ""))
  } else {
      df <- queryDB(conn, 
            paste("SELECT RelSide, RelHeight, 
                          PlateLocSide, PlateLocHeight,
