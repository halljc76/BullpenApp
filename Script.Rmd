...
addTableDB <- function(conn, filename, tablename) {
  ...
  tables <- unname(as.list(queryDB(conn, "SELECT name FROM sqlite_master")))
  if (!(tablename %in% tables)) {
    ...
    suppressWarnings( 
      expr = {
        cols <- names(queryDB(gsub(pattern = "_", replacement = tablename, x = "SELECT * FROM _")))
        for (col in cols) {
...
addTableDB(conn, "TestData.csv", "TestData")
